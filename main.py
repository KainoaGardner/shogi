                ...
                if event.key == pygame.K_r:
                    if len(board.moveHistory) > 0:
                        ...
                        if move[0][0][0] >= 0:
                            ...
                            if move[1][1][1] in board.promotePieceList:
                                piece = board.promoteDict.get(move[1][1][1])
                                index = board.pieceList.index(piece)
                                if board.sente:
                                    if board.goMochigoma.count(piece) > 0:
                                        board.goMochigoma.remove(piece)
                                        board.goMochigomaCount[index] -= 1
                                if not board.sente:
                                    if board.senMochigoma.count(piece) > 0:
                                        board.senMochigoma.remove(piece)
